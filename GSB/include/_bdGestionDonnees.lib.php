...
function obtenirReqMoisFicheFrais($unIdVisiteur) {
    $req = "SELECT FicheFrais.mois as mois FROM FicheFrais WHERE FicheFrais.idvisiteur ='" . $unIdVisiteur . "' ORDER BY FicheFrais.mois desc";
    return $req ;
}  
...
function obtenirReqEltsHorsForfaitFicheFrais($unMois, $unIdVisiteur) {
    $unMois = filtrerChainePourBD($unMois);
    $requete = "SELECT Distinct id, date, libelle, montant FROM LigneFraisHorsForfait INNER JOIN LigneFraisForfait
...
function modifierLigneHF($idCnx, $unIdLigneHF, $unIdVisiteur) {
    $requete = "update LigneFraisHorsForfait set libelle = 'REFUSE' 
		where idVisiteur ='" . $unIdVisiteur . "' and id = '". $unIdLigneHF . "'";
    mysql_query($requete, $idCnx);
}
 ...
 * @return void
 */
function ajouterLigneHF($idCnx, $unIdVisiteur, $uneDateHF, $unLibelleHF, $unMontantHF) {
